...
A continuaci贸n se eliminar谩n las variables que se consideran innecesarias para el modelo

#### Gr谩fica de correlaci贸n de las variables
...
corPlot(pricesTrainGraph) # Correlaci贸n de todas las variables num茅ricas
...
De este modo podemos ver que las variables BsmtFinSF2, LowQualFinSF, BsmtHalfBath, , GarageYrBlt, X3SsnPorch, MiscVal no tienen nig煤n tipo de correlaci贸n y podr铆an afectar de manera negativa en el modelo, de modo que se eliminar谩n as铆 como la mayor铆a de variables cualitativas. En total se eliminar谩n las siguientes variables : *Id, MSSubClass, Street, Alley, LotShape, LandContour, LotConfig, LandSlope, Neighborhood, Condition1, Condition2, BldgType, RoofStyle, RoofMatl, Exterior1st, Exterior2nd, MasVnrType, BsmtQual, BsmtCond, BsmtExposure, BsmtFinType1, BsmtFinType2, Heating, Electrical, KitchenQual, FireplaceQu, GarageFinish, GarageQual, PavedDrive, PoolQC, Fence, MiscFeature, SaleType, SaleCondition, BsmtFinSF2, LowQualFinSF, BsmtHalfBath, , GarageYrBlt, X3SsnPorch, MiscVal*
...
\## COEFICIENDE DE DETERMINACI覰:

El coeficiete de determinaci髇 del modelo es \$r\^2 = 0.4877\$, lo cual indica que el 48.7% de la variabilidad del fen髆eno esta siendo explicado por el modelo generado.

El \$r\^2\$ ajustado se encuentra bastante cercado al coeficiente de determinaci髇, por lo que la colinealidad no es un problema que nos deba preocupar.

\## AN罫ISIS DE RESIDUALES
...
El test anterior nos brinda un \$valor-p =2.2\\times10\^{-16} \< 0.05\$, por lo que tenemos evidencia suficiente para rechazar la hip髏esis nula, y con una significancia de 0.05, los residuales no tienen un comportamiento normal. Por lo que no se podria recomendar la utilizaci髇 del modelo generado.
...
Utilizaremos la prueba de Durbin Watson, la cual mide la autocorrelaci髇 entre los residuales.

\$\$Ho: \\text{Los residuos estan autocorrelacionados}\$\$

\$\$Ha: \\text{No existe autocorrelaci髇 en los residuales}\$\$

```{r}
dwtest(modelo)
```

El \$valor-p=9854 \>0.05\$ indica que los residuales est醤 autocorrelacionados, por lo que podemos afirmar que los residuales no son independientes. por lo cual no es recomendado utilizar el modelo
...
Con un \$valor-p = 1\$ no hay evidencia para rechazar la hip髏esis nula, por lo que parece razonable afirmar que la media de los residuales es igual a cero.
...
Dada la correlaci贸n entre las variables evidenciada en la gr谩fica, es necesario prescindir de las variables YearsBuilt, YearsRem, GrLivArea, FullBath, GarageCars, GarageArea, BsmtUnfSF, BsmtFullBath, X1stFlrSF, X2ndFlrSF, BedroomAbvGr.
...
De la funci贸n anterior tenemos que el mejor modelo es el de la forma
...
```
Es discutible el hablar de normalidad utilizando la gr谩fica anterior, pues en los datos del centro pareciera existir, pero los puntos de los extremos se alejan demasiado de la l铆nea de los 45掳. Por lo que se realizar谩 una prueba de Lilliefors para determinar si existe dicha normalidad
...
```
En la gr谩fica se puede observar que los residuales siguen un patr贸n bien definido, lo cual es un claro indicador de la ausencia de independencia.

###### Independencia de los residuales
Utilizaremos la prueba de Durbin Watson, la cual mide la autocorrelaci贸n entre los residuales.

$$ H_0:  \text{Los residuales estan autocorrelacionados}$$
$$ H_1: \text{No existe autocorrelaci贸n en los residuales}$$


```{r independencia}
dwtest(mdLnMtvS)
```
El test anterior devuelve un $valor-p = 0.8921 > \alpha = 0.05 \implies H_0$ no puede ser rechazada, esto indica que los residuales est谩n autocorrelacionados, por lo que podemos afirmar que los residuales no son independientes.
...
```
Podemos ver que el error medio cuadrado es de 1303358790, lo cual confirma la conclusi贸n anterior que el modelo no es adecuado para la predicci贸n del precio de las casas.
